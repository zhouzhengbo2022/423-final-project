...
```{r, echo=FALSE, results='hide', message=FALSE, error=FALSE, warning=FALSE}
dataset <- read.csv("/Users/miley/Desktop/2021 Spring/Data science/project/ML/Facebook_metrics/dataset_Facebook.csv", sep=";", header=T) # import data
...
set.seed(1)
boost.dataset=gbm(Page.Total.Likes~.,data=dataset, distribution= "gaussian",n.trees =500, interaction.depth =4)

summary(boost.dataset)

##partialPlots
par(mfrow=c(1,2))
plot(boost.dataset ,i="Post.Month")
plot(boost.dataset ,i="Reach.by.Users.with.Likes")
plot(boost.dataset ,i="Paid")
...
```{r}
#ridge model
x = model.matrix(Page.Total.Likes ~ ., dataset)
